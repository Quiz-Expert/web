<template>
  <main class="flex justify-center py-12 flex-1 overflow-x-hidden overflow-y-auto">
        ...
        <div class="shadow-xl overflow-hidden border-b border-gray-200 rounded-lg">
          <table class="w-full divide-y-2 divide-green-600">
            <thead class="bg-gray-50">
              <tr>
                <th scope="col"
                    ...
                    v-text="$t('pages.dashboard.suggestions-panel.category')"
                />
                <th scope="col"
                    class="px-2 sm:px-4 md:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr v-for="suggestion in suggestions" :key="suggestion" class="hover:bg-gray-100">
                <td class="px-2 sm:px-4 md:px-6 py-4 whitespace-nowrap">
                  <div class="flex items-center">
                    <div class="text-sm font-medium text-gray-900" v-text="suggestion.name" />
                  </div>
                </td>
                <td class="px-2 sm:px-4 md:px-6 py-4 whitespace-nowrap">
                  <div class="flex items-center">
                    <div class="text-sm font-medium text-gray-900" v-text="suggestion.description" />
                  </div>
                </td>
    ...
    <EditSuggestion v-show="isEditModalVisible"
                    @close="isEditModalVisible=false"
    />
    <DeleteModal v-show="isDeleteModalVisible"
                 :tittle="$t('pages.dashboard.suggestions-panel.removal.tittle')"
                 :question="$t('pages.dashboard.suggestions-panel.removal.question')"
  ...
  data() {
    return {
      ...
      suggestions: [
        ...
        {
          name: "Kategoria - 1",
          description: "Opis Pytania 3"
        },
        {
          name: "Kategoria - 2",
          description: "Opis Pytania 4"
        },
      ]
    }
  }
