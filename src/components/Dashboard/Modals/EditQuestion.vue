...
export default {
  ...
  computed: {
    ...mapGetters(["allCategories", "questionById", "isMistakeExist"]),
  },
  ...
  mounted() {
    ...
    this.$store.dispatch("GET_QUESTION_BY_ID", this.questionId)
      .then(() => {
        ...
        this.questionCategory = this.questionById.category;
      });
  },
    ...
    updateQuestion() {
      ...
      this.$store.dispatch("UPDATE_QUESTION", this.questionData)
        .then(() => {
          this.close();
          this.$emit('edit');
          this.$store.dispatch("SUCCESS_NOTIFICATION", this.$t('pages.dashboard.questions-panel.edition.message'));
        })
        .catch(err => {
          console.log(err);
